...
\begin{titlepage}
  ...
  \medskip
  Correspondence: \texttt{dvirross@shenkar.ac.il} or \texttt{RossDv@sce.ac.il}
  \end{flushleft}
\end{titlepage}
...
\begin{description}
  \item[$\HG$ \textnormal{(global exchangeability).}] All 1,554 haflaga values are
  ...
  \item[$\HI$ \textnormal{(multinomial i.i.d.).}] Each woman's $n_i$ values are drawn
  ...
  \item[$\HW$ \textnormal{(within-woman exchangeability).}] Conditional on each woman's
...
\begin{equation}
  ...
  \quad b_j^+ = \#\{r : S_j^{(m,r)} \geq O_j\},\;
  b_j^- = \#\{r : S_j^{(m,r)} \leq O_j\},
  \label{eq:ptwo}
\end{equation}
...
\paragraph{Pairwise dependence (exploratory).}
Spearman rank correlations were computed between per-woman pattern \emph{rates}
(events divided by cycles), which reduce but do not remove the exposure difference
between women with few and many cycles. Two-sided permutation p-values ($50{,}000$
permutations of one rate vector) with Holm correction over the 10 pairs are reported;
a binary (ever/never) version with Fisher's exact test serves as a robustness check.
...
\midrule
\multicolumn{8}{l}{Joint (five patterns): $\DM = 2.93$, $p = .126$ (6,285 of 50,000
  replicates at or beyond); split-batch $\DM = 2.95$, $p = .119$; LOO range $.055$--$.224$} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Exploratory three-pattern subspace: 3,000 null replicates (points) and the
    observed vector $(26, 18, 27)$ in the (Haflaga, Week-Dilug, Dilug-in-Dilug) space
    under $\HG$ (top left), $\HI$ (top right) and $\HW$ (bottom). Marker size encodes
    the Dilug count and shade the Week count; the filled circle is the null mean and the
    dashed segment the displacement. Primary inference uses all five patterns
...
Section~\ref{sec:mathdep} predicts positive association for Haflaga $\times$ Week-Dilug
(by containment --- an arithmetic consequence of the definitions, not an empirical
hypothesis) and makes positive association for Haflaga $\times$ Dilug-in-Dilug plausible
(by proximity). No prediction is made for the other eight pairs. Table~\ref{tab:chisq} and
Figure~\ref{fig:chisq} give the rate-based Spearman correlations. Haflaga $\times$
Dilug-in-Dilug is positive and survives Holm correction ($r_s = 0.27$,
$p_{\text{perm}} = .004$, Holm $p = .038$); Haflaga $\times$ Week-Dilug is positive but
does not ($r_s = 0.23$, $p_{\text{perm}} = .011$, Holm $p = .10$); the remaining pairs
are null (all Holm $p = 1$). The binary version gives the same two smallest p-values
(Haflaga $\times$ Dilug-in-Dilug: $\chi^2 = 9.99$, $p_{\text{perm}} = .004$, Holm
$p = .042$, Fisher $p = .004$; Haflaga $\times$ Week-Dilug: $\chi^2 = 7.26$,
$p_{\text{perm}} = .013$, Holm $p = .12$). These associations are between per-woman
rates, are subject to the exposure difference between women with 5 and 45 cycles, and
are compatible with --- not evidence for --- the proximity mechanism, which would
require an explicit generative model to test.
...
\begin{figure}[H]
  ...
  \caption{Frequency of each haflaga value $H_k$ in the analysed sample. Counts decline
    smoothly across the Week-Dilug anchor ($H = 29$: 225; $30$: 175; $31$: 141).}
  \label{fig:heaping}
\end{figure}
