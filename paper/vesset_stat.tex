...
\begin{description}
  ...
  \item[$H_W$ \textnormal{(within-woman permutation):}] Each woman\'s cycles permuted uniformly at random within her own sequence (without replacement; observation indices are permuted, not unique values, so ties are handled correctly). Tests whether temporal \emph{ordering} creates excess
...
the result is highly significant under the global null and biologically
interpretable: Haflaga detects the pattern expected in women with lower
within-person cycle-length variability. Women who establish Haflaga have within-person
SDs of 1.54 days ($n = 20$) vs.\ 3.17 days for women who establish no vesset at all
($n = 35$; Mann--Whitney $p < .001$, Cohen's $d = -1.13$; see Section~\ref{sec:data}).
...
derivable from the pattern definitions alone (Section~\ref{sec:mathdep}). To our
knowledge, this has not previously been stated explicitly in the halachic or
statistical literature.

The three key relationships are: (i)~\textbf{containment} --- every Haflaga-30
sequence is also a Week-Dilug event by set-theoretic inclusion, so 3 of the 26 Haflaga
cases count deterministically towards Week-Dilug; (ii)~\textbf{limiting convergence}
--- a Dilug-in-Dilug sequence with small second difference $|d| = 1$ deviates from
a constant by at most a few days, placing it behaviourally close to Haflaga; 16 of
the 27 observed DiD sequences have $|d| = 1$, explaining the strong positive association
($\chi^2 = 9.99$, $p_{\text{perm}} = .004$); (iii)~\textbf{mutual exclusion at the
...
These structural constraints have a direct implication for inference. The five pattern
counts are non-independent by construction, so Bonferroni--Holm correction --- though
valid under any dependence --- is conservative when tests are positively correlated.
The focused 3-pattern joint Mahalanobis test (Section~\ref{sec:joint}) evaluates the
structurally linked count vector as a single entity, avoiding multiple-comparison
correction entirely and exploiting the positive dependence among the three patterns.
...
are identical whether computed on $H_k$ or $L_k$ provided the anchor shifts by the
same $-1$ (verified: Haflaga 26, Week-Dilug 18 under both conventions).
The raw haflaga distribution shows no heaping artefact at round numbers: counts
decline smoothly from the mode ($H = 29$: 225 cycles) through the Week-Dilug anchor
($H = 30$: 175) to $H = 31$ (141), consistent with the known biological mode rather
than rounding (Figure~\ref{fig:heaping}).
...
expected to be approximately uniform across women and pattern types.
...
The pairwise association analysis (Table~\ref{tab:chisq}) is subject to a
cycle-count confound: women with more cycles have more opportunities to establish
any pattern; associations involving rare patterns (Week-Dilug: 18 women;
Dilug-in-Dilug: 23 women) should be interpreted with particular caution. The
Haflaga $\times$ Dilug-in-Dilug association that survives Holm correction is at least
partly mathematically entailed (polynomial proximity), providing structural
...
as an anticipatory pattern. The joint distance-based permutation test, adapted here for dependent count vectors
...
aggregate correlated count evidence without multiple-comparison correction. When
multiple count outcomes arise from shared biological structure --- as they do here
via the finite-difference hierarchy --- aggregating into a single Mahalanobis
statistic avoids the conservatism of FWER correction under positive dependence.
This approach may prove useful for composite hypothesis testing in other cyclic
biological settings (e.g., multitype rhythm patterns, circadian or hormonal cycle data)
where structurally related outcomes are tested jointly.
