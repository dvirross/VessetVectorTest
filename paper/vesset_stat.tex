...
\begin{itemize}
  ...
  \item \textbf{Structural basis for exclusion.} Dilug (degree-1) and Week (degree-0
    ...
    relationship with the three included patterns. Dilug yields a non-significant marginal result (Holm adj $p = .346$) with no analytical prediction
...
(ii) the non-significant $H_W$ result (all Holm-corrected $p \geq .198$; joint $D_M = 2.95$, $p = .121$);
(iii) the primary 3-pattern Mahalanobis joint test under the global null ($D_M = 4.52$, $p < .001$).
\textbf{Exploratory:} Dilug ($p = .087$, Holm adj $p = .346$; hypothesis-generating only), pairwise dependence tests, and the focused joint test (motivated structurally but secondary).
...
$\alpha = .05$ (two-sided), using a normal-theory location-shift approximation:
\begin{equation}
