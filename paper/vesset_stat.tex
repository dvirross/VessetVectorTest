...
cycles analysed with the same code and settings (global $\DM = 9.99$; within-woman $\DM = 1.20$,
...
\begin{description}
  ...
  \item[\textit{Week-Dilug.}] $H_k = H_{k-1} = 30$. An interval of 30 (29 days, four weeks
...
Week-Dilug requires two consecutive intervals equal to 30; a Haflaga sequence with
...
the observed $\DM$ unchanged ($5.08$, $4.87$ and $2.94$ under $\HG$, $\HI$ and $\HW$)
and moves the joint $p$-values from $< .001$, $< .001$ and $.126$ to $< .001$, $< .001$
and $.150$, because every null replicate that contains an event lies beyond the observed
...
non-significant in every stratum ($p = .37$, $.79$ and $.26$), and the women who produce
the most events are those the null expects to: the rank correlation between a woman's
observed and expected event totals is $0.62$, no woman departs from her own null after
...
and Dilug-in-Dilug Holm-adjusted $p$ $.021$--$.024$) and $\DM = 9.99$
(Table~\ref{tab:ext}). Under $\HW$ every $|z| \leq 1.13$, all two-sided $p \geq .31$, and
$\DM = 1.20$ ($p = .92$; split-batch $1.20$, $.92$); the Dilug-in-Dilug deficit seen in
the Marquette data is absent ($z = -0.08$). The conclusion is unchanged by leaving any
woman out (joint $p$ $.86$--$.97$), truncation to twelve cycles ($\DM = 1.34$, $p = .88$)
or restriction of the support to 18--54 days ($\DM = 1.15$, $p = .93$); empirical size is
...
Section~\ref{sec:within} also hold: every regularity stratum is null (joint $p \geq .49$),
...
\midrule
$\HG$  & 5.08 ($<.001$) & 9.99 ($<.001$) & $+8.33$ \\
$\HI$  & 4.87 ($<.001$) & 9.53 ($<.001$) & $+8.20$ \\
$\HW$  & 2.93 ($.126$)  & 1.20 ($.921$)  & $+1.13$ \\
\bottomrule
\end{tabular}
\end{table}
