...
\begin{titlepage}
  ...
  \medskip
  The dataset analysed in this study was collected by \citet{fehring2013} and is
  publicly archived at Marquette University \citep{marquette2013data}. The author
  declares no conflicts of interest. Analysis code, the filtered dataset and cached
  Monte Carlo replicates are available at
  \url{https://github.com/dvirross/VessetVectorTest}.

  \medskip
  Article type: Case Study. Correspondence: \texttt{dvirross@shenkar.ac.il} or \texttt{RossDv@sce.ac.il}
  \end{flushleft}
\end{titlepage}
...
\noindent
Jewish religious law (\textit{Halacha}) prescribes that a woman anticipate menstruation
from recurring arithmetic patterns in her cycle history (the \textit{vesset} rules).
Each rule defines an exact event in a sequence of inter-menstrual intervals --- three
equal consecutive intervals, a constant first or second difference, a repeated weekly
anchor --- and the natural question is whether such events occur in real data more often
than chance. The statistical difficulty is that the answer depends on what ``chance''
means for clustered longitudinal sequences whose rule-defined counts are dependent by
construction. We analyse 1,554 cycles from 118 women under three exchangeability nulls
...
means. A simulation study confirms nominal size for all tests, quantifies the power of
the within-woman tests against autoregressive and exact-repetition alternatives, and
shows the conclusions are insensitive to unequal follow-up. The contrast is consistent
with between-woman heterogeneity in regularity generating the global excess, and
provides no evidence that within-woman ordering produces additional rule-defined
events; it does not establish the absence of serial dependence.
...
datasets, we are not aware of any formal randomisation-based test of whether these
events occur more often than chance in empirical data (no systematic literature search
was performed).
...
exceed a null expectation, when (i) subjects contribute sequences of very unequal length
...
within subjects. The choice of null is therefore not a technicality. Permutation
inference for longitudinal and clustered data has been studied extensively
\citep{friedrich2017,lee2012}, exchangeability-block permutation is standard in
...
Schneider, two of the investigators of the dataset analysed here \citep{fehring2013}.
...
We analysed the publicly archived longitudinal dataset of \citet{fehring2013}, collected
in a randomised trial of two internet-supported natural family planning (NFP) methods
\citep{marquette2013data}. The archived filtered file retains cycles of 18--54 days with
complete cycle-length records \citep{creinin2004}; the analysed sample comprises
...
skewness $= 1.29$); 9.6\% of cycles were 35 days or longer. Women were aged 21--43 years
($M = 30.6$, SD $= 5.7$); parity was not recorded. The data come from two charting
...
the cycle exclusions could have created artificial adjacency across a removed cycle. In
the filtered file every woman's recorded \texttt{CycleNumber} runs $1, 2, \ldots, n_i$
without gaps (verified programmatically and asserted in the analysis code), so no pattern
window spans an excluded cycle; the exclusions removed whole women or terminal cycles
only. The minimum of 5 cycles per woman is the observed minimum after filtering; no
additional minimum was imposed. The script that produced the filtered file from the raw
archive was not preserved and is not part of the code release; the filtered file itself
is archived with the code.

Of the seven rules computable from cycle lengths, the two repeating-cycle rules
(\textit{Dilug Chozer Chalila}, \textit{Haflaga Chozer Chalila}) had zero occurrences,
and the analysed family of five was fixed after this was observed. We state this
plainly: the family is data-informed, not pre-specified. Its consequences are limited ---
a zero-count component cannot be included in a Mahalanobis statistic (its null
covariance is degenerate at the observed sequence lengths), and adding two marginal
tests with observed count 0 would not alter any conclusion below --- but the family
should be read as primary rather than confirmatory.
...
\begin{figure}[!htbp]
  ...
  \caption{Participant and data flow. Source dataset \citep{fehring2013}; inclusion
    criteria (cycle length 18--54 days; complete records); analysed sample of 118 women
    and 1,554 cycles; five pattern types; three null models, each with
    $B = 50{,}000$ replicates.}
  \label{fig:consort}
\end{figure}
...
null sequences. The observed count vector is
...
\subsection{Pairwise Dependence Analysis (Exploratory)}
\label{sec:pairmethods}

The joint test treats the five counts as one vector; the pairwise analysis asks a
different and secondary question --- whether women who produce one pattern type tend to
produce another --- as a consistency check against the relationships derived in
Section~\ref{sec:mathdep}. Each design choice below follows from a specific feature of
the data.
...
exchangeable unit for an association test. The analysis therefore works with 118
per-woman summaries, not with 1,554 cycles.
...
exposure effect. It does not remove it entirely --- a rate from 5 cycles is far noisier
than one from 45, and the minimum run lengths (2, 3 or 4 cycles) mean short sequences
have proportionally fewer eligible windows --- which is one reason the analysis is
...
\paragraph{Permutation rather than asymptotic p-values.} With 118 women, many tied
zeros and skewed rates, the usual $t$-approximation for the null distribution of $r_s$
is unreliable. Instead one rate vector is permuted across women $50{,}000$ times; this
breaks any association between the two patterns while preserving both marginal
distributions exactly, giving a two-sided Monte Carlo p-value (proportion of
permutations with $|r_s^*| \geq |r_s|$, with the $(b+1)/(B+1)$ convention).
...
tests, which matters here because the pairs share patterns.
...
each woman coded 1/0 for ever having established each pattern. This discards magnitude
entirely. Each pair is then a $2 \times 2$ table tested with Fisher's exact test
(conditional on the margins) and with a permutation $\chi^2$ statistic (permuting one
indicator across women), again Holm-corrected. Agreement between the rate-based and
binary results indicates that the conclusions do not depend on the choice of metric.

\paragraph{What a positive association can and cannot mean.} For Haflaga $\times$
Week-Dilug a positive association is partly guaranteed: every Haflaga sequence at
$H = 30$ contains a Week-Dilug event, so the test partly re-discovers the definitions.
For Haflaga $\times$ Dilug-in-Dilug the definitions do not entail co-occurrence, so an
association is an empirical observation about the women in this sample; it is compatible
with the proximity argument but would require a generative model to attribute to it. In
neither case does the pairwise result bear on the primary question of whether ordering
...
procedures of the main analysis.
...
$\HW$ null at 31.8, above the observed 26. The reason is that women with narrow
cycle-length multisets generate many exact repetitions under \emph{any} ordering of their
values; conditioning on the multiset absorbs this.
...
\begin{figure}[!htbp]
  ...
  \caption{Exploratory three-pattern subspace: 3,000 of the 50,000 null replicates
    (points) and the observed vector $(26, 18, 27)$ in the (Haflaga, Week-Dilug,
    Dilug-in-Dilug) space under $\HG$ (top left), $\HI$ (top right) and $\HW$ (bottom),
    so that all five counts are visible at once: marker size encodes the Dilug count
    (size legend in each panel) and marker shade the Week count (colour bar). The filled
    circle is the null mean; the dashed segment is the displacement to the observed
    vector, with its Euclidean length $D_E$ and Mahalanobis length $D_M$ in the legend.
    Primary inference uses all five patterns (Table~\ref{tab:stratified}).}
  \label{fig:joint3panels}
\end{figure}

\subsection{Pairwise Dependence (Exploratory)}
\label{sec:chisq}

The methods and their rationale are given in Section~\ref{sec:pairmethods}. Section~%
\ref{sec:mathdep} leads to two expectations: a positive Haflaga $\times$ Week-Dilug
association, which is partly an arithmetic consequence of containment rather than an
empirical hypothesis, and a plausible but not entailed positive Haflaga $\times$
Dilug-in-Dilug association from polynomial proximity. No expectation is held for the
other eight pairs, so the analysis is a consistency check, not a search.

Table~\ref{tab:chisq} and Figure~\ref{fig:chisq} give the rate-based Spearman
...
which ($|d| = 1$ in 16 of 27) are near-constant sequences. Haflaga $\times$ Week-Dilug
is positive, as containment requires, but does not survive correction ($r_s = 0.23$,
...
are null (all Holm $p = 1$), including Dilug $\times$ Week-Dilug, whose weak negative
sign ($r_s = -0.12$) is in the direction implied by their mutual exclusion but is not
...
normalisation or on the few women with multiple events. Three cautions apply. The
associations are between women, not within sequences, and say nothing about temporal
ordering. They remain exposed to the residual exposure difference between women with 5
and 45 cycles (Section~\ref{sec:pairmethods}). And the surviving association is
compatible with the proximity mechanism but is not evidence for it: attributing
co-occurrence to a mechanism would require an explicit generative model for the
sequences, which this paper does not fit.
...
\caption{Pairwise Spearman correlations between per-woman pattern rates ($N = 118$),
  ...
  (Section~\ref{sec:mathdep}). Bold: $p_{\text{Holm}} < .05$.}
\label{tab:chisq}
\smallskip
...
Every procedure holds its nominal level: the two-sided marginal tests reject between
2.9\% and 4.8\% of the time (below nominal because the counts are discrete), the Holm
family between 3.2\% and 3.7\%, and the joint test between 4.8\% and 5.3\% at
$\alpha = .05$ and between 0.7\% and 1.1\% at $\alpha = .01$. The plug-in and
...
AR(1) $\rho$       & 0    & $-0.1$ & .04 & .05 & .04 & .04 & .04 & .03 & .06 \\
                   & 0.25 & $+5.0$ & .19 & .24 & .10 & .06 & .18 & .26 & .51 \\
                   & 0.50 & $+13.8$ & .76 & .74 & .35 & .31 & .44 & .89 & .995 \\
                   & 0.75 & $+33.3$ & 1.00 & .97 & .78 & .67 & .73 & 1.00 & 1.00 \\
\midrule
Persistence $q$    & 0    & $+0.2$ & .04 & .04 & .04 & .04 & .06 & .05 & .06 \\
                   & 0.1  & $+24.2$ & .96 & .19 & .61 & .41 & .04 & .94 & .98 \\
                   & 0.2  & $+52.7$ & 1.00 & .60 & .96 & .86 & .04 & 1.00 & 1.00 \\
                   & 0.3  & $+80.4$ & 1.00 & .93 & .99 & .96 & .17 & 1.00 & 1.00 \\
\bottomrule
\end{tabular}
\end{table}
...
These results calibrate the main finding. Had within-woman ordering produced an excess
of roughly five Haflaga events in this sample, the joint test would have detected it
about half the time; an excess of 14 or more would almost certainly have been detected.
The observed Haflaga count is six \emph{below} its conditional null mean. Serial
dependence weaker than $\rho \approx 0.25$, or dependence that does not manifest as
whole-day autoregression or exact repetition, may go undetected by these statistics.
...
Dilug-in-Dilug deficit under $\HW$ (Holm $p = .19$), the pairwise associations, and the
three-pattern subspace. \emph{Robustness:} the multinomial null, the split-batch joint
...
repeating-cycle rules were dropped after zero occurrences were observed. The sample
consists of non-Jewish, regularly cycling, married NFP users aged 18--42
\citep{fehring2013}, a self-selected, cycle-aware group who may be more regular than
...
The simulations (Section~\ref{sec:simresults}) show high power against exact-repetition
mechanisms and against whole-day autoregression of moderate strength ($\rho \geq 0.5$),
but only about 50\% power at $\rho = 0.25$ and, necessarily, none against dependence that
leaves the distribution of these exact count statistics unchanged; the non-rejection
should be read with those limits. The
aggregate-count estimand weights women by follow-up; the truncation and
leave-one-woman-out analyses indicate that this does not drive the conclusions. Sensitivity
to the 18--54 day inclusion window could not be assessed because the excluded cycles are
not in the archived file. Pairwise associations involving rare patterns (18 and 27
events) rest on few women and are exploratory.
...
far in the tail, driven by a fourfold excess of Haflaga events; under a within-woman
permutation null conditional on each woman's own cycle-length values it is not, and all
counts lie at or below their conditional means. Simulation confirms that every test used
holds its nominal size, that the within-woman tests have high power against
exact-repetition alternatives and against autoregressive dependence of moderate strength
(about 50\% at $\rho = 0.25$, near-certain at $\rho \geq 0.5$), and that the conclusions
are insensitive to unequal follow-up. The results are consistent with
...
This study is a secondary analysis of a publicly archived dataset
\citep{fehring2006,fehring2013}. The original data collection was approved by the
Institutional Review Board at Marquette University. No new data were collected, and no
individual participant can be identified from the released dataset. Separate ethics
approval for secondary analysis was not required under the applicable regulations.
...
The menstrual cycle dataset is publicly available from the Marquette University Natural
Family Planning repository \citep{marquette2013data}. The filtered data file, all analysis
code (pattern counting with an equivalence test between two implementations, null
generators, simulation validation, post-processing), the Jupyter notebook that produces
every figure, and the cached Monte Carlo replicates ($B = 50{,}000$ per null) that allow
every reported number to be reproduced without re-simulation are available at
\url{https://github.com/dvirross/VessetVectorTest}.

\section*{AI Assistance Disclosure}

Portions of the text were drafted and edited with the assistance of Claude (Anthropic),
a large language model, and the tool was also used to generate and refactor \LaTeX{} and
\texttt{Python} code under the author's direction. All scientific content, analytical
decisions and interpretations are the author's, who takes full responsibility for the
