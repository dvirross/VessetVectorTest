...
\begin{titlepage}
  ...
  {Dvir Ross\par}
  \vspace{0.2cm}
  {\small ORCID: 0000-0001-8217-3359\par}
  \vspace{0.4cm}
  {\normalsize\itshape
...
\medskip
\noindent\textit{Keywords:} Halacha, Mahalanobis joint test, menstrual cycle, permutation test, within-woman permutation
...
present study. The multinomial sampling null \cite{ross2022phd} draws each
...
theoretical comparison is planned in future work \cite{ross2022phd}.
...
the absence of analytical predictions for those pairs. Figure~\ref{fig:chisq} visualises the full correlation structure.

As a robustness check, a binary co-occurrence analysis was conducted --- coding each woman
1/0 for whether she ever established each pattern, and testing all 10 pairs with
Fisher's exact test plus permutation $p$-values --- and yielded the same two lowest
...
of metric. The Haflaga $\times$ Dilug-in-Dilug binary permutation $p$-value (0.004) falls below 0.01,
...
This study is a secondary analysis of a publicly archived dataset (Fehring, Schneider,
\& Raviele, 2006; Fehring, Schneider, Raviele, \& Barron, 2013). The original data
collection was approved by the Institutional Review Board at Marquette University. No
new data were collected for the present study, and no individual participant can be
identified from the publicly released dataset. Separate ethics approval for secondary
analysis of this dataset was not required under the applicable regulations.
...
The menstrual cycle dataset analysed in this study is publicly available from the
Marquette University Natural Family Planning repository (Fehring et al., 2013). The
analysis code, simulation scripts, and figure generators are available at
\url{https://github.com/dvirross/VessetVectorTest}. Pre-computed simulation results
(50,000 permutations per null model) are archived in the repository to support
exact reproducibility without re-running computationally intensive simulations.
...
and generation of \LaTeX{} and \texttt{Python} code under the author's direct supervision
