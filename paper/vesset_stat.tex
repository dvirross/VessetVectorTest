...
\paragraph{Cycle-length autocorrelation.}
Recent large-scale analyses have detected statistically significant autocorrelation in
menstrual cycle lengths. Cortet et al.\ \cite{cortet2024} identified second- to
fourth-order autocorrelation in a sample of over 30,000 cycles, establishing that
cycle lengths are not serially independent at the population level. The within-woman
exchangeability null ($H_W$) adopted here conditions on each woman's observed
cycle-length multiset; it does not assume serial independence and remains valid in the
presence of autocorrelation, provided such autocorrelation does not systematically
produce additional exact vesset events within the permuted sequences.
