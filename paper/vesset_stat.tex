...
the counts are integer-valued and dependent by construction. We
...
vector is displaced far into the tail ($\DM = 5.07$, $p < .001$; Haflaga $z = 4.50$).
Under the within-woman null it is not ($\DM = 2.97$, $p = .11$; smallest Holm-adjusted
...
contrast reproduced in an independent, openly deposited cohort of 270 women and 2,139
cycles analysed with the same code and settings (global $\DM = 10.04$; within-woman $\DM = 1.22$,
$p = .92$). The contrast is consistent with between-woman heterogeneity in regularity generating the
...
rule types in the halachic literature, 11 require calendar or time-of-day data that
standard cycle records do not contain; 6 are computable from interval lengths alone.
...
frequency of these events in empirical data departs from what chance would produce; a
search of the halachic and biostatistical literatures conducted for the author's
dissertation and repeated in September 2026 found none (Supporting Information,
Section~S2).
...
(here 5--45 cycles), (ii) the counts are integer-valued and small (here 18--61), (iii) the counts are
...
ross2026statistical,ross2022phd} and is presented as a Case Study.
...
\textit{interval} $H_k = L_k + 1$ from the first day of menstruation $k$ to the first
day of menstruation $k+1$; in halachic terminology this quantity is the \textit{haflaga},
and we follow that convention for its value \citep{ross2026statistical}. Throughout,
``interval'' denotes $H_k$, and the capitalised pattern name \textit{Haflaga} below
denotes the rule that three consecutive intervals are equal. The five rule types analysed
here are:

\begin{description}
  \item[\textit{Haflaga.}] $H_k = H_{k-1} = H_{k-2}$: three or more consecutive equal
    intervals. Degree-0 (constant) sequence; minimum run 3 cycles.
  ...
  \item[\textit{Week.}] $H_k = H_{k-1} \in \mathcal{W}$, where
    $\mathcal{W} = \{7n + 1 : n \in \mathbb{N}^{+}\}$ is the set of intervals
    whose cycle length $L = H - 1$ is a multiple of 7, so that successive onsets fall on
    the same weekday; in these data $\mathcal{W} \cap [19, 55] = \{22, 29, 36, 43,
    50\}$. Degree-0 anchored to weekly values; minimum run 2.
  \item[\textit{Week-Dilug.}] $H_k = H_{k-1} \in \mathcal{W}'$, where
    $\mathcal{W}' = \{7n + 2 : n \in \mathbb{N}^{+}\} = \{23, 30, 37, \ldots\}$ is the
    set of intervals whose cycle length $L = H - 1$ exceeds a whole number of weeks by
    one day, so that each successive onset falls one weekday later, producing an
    arithmetic progression of weekdays --- hence the name (Hebrew \textit{shavu'a
    be-dilug}; \citealp{ross2022phd}). Only the one-weekday progression is recognised in
    the halachic sources; a progression of $k$ weekdays for arbitrary $k$ would make every
    pair of equal intervals a Week-Dilug event and the rule would collapse into a two-cycle
    Haflaga. In these data every observed Week-Dilug event is at $H = 30$ (29 days, four
    weeks plus one day), the dominant anchor in practice. Degree-0 anchored to weekly
    values; minimum run 2.
\end{description}
...
values. The sixth rule computable from interval lengths alone, \textit{Haflaga Chozer
Chalila} (a block of at least three intervals repeated exactly three times in
succession, hence a window of at least nine cycles), had no observed occurrence; it is
discussed in Section~\ref{sec:data} and evaluated under the three nulls in the
...
\paragraph{Haflaga at a Week-Dilug anchor $\subseteq$ Week-Dilug (deterministic
containment).}
Week-Dilug requires two consecutive equal intervals at a value in $\mathcal{W}'$; a
Haflaga sequence at such a value requires three. Every Haflaga sequence at a Week-Dilug
anchor therefore generates a Week-Dilug event in its first two cycles. This is
set-theoretic inclusion, not a stochastic association: in the data, the 3 Haflaga
sequences at an anchor value (all at $H = 30$) contribute directly to the Week-Dilug
count of 18, all of whose events are at $H = 30$.
...
differences change by one day per cycle and the interval sequence is nearly constant. In
...
Because the detectors operate on \emph{consecutive} cycles, we verified that the
exclusions created no artificial adjacency: only whole women or duplicated rows were
removed, and every retained woman's \texttt{CycleNumber} runs $1, 2, \ldots, n_i$
without gaps (asserted in the analysis code). The five-cycle minimum ensures that every retained woman can in
...
Of the six rule types computable from cycle-length data, \textit{Haflaga Chozer
Chalila} had no observed occurrence. Because a zero count is informative only relative
to what the nulls expect, the rule was implemented and evaluated under the same three
nulls with $B = 50{,}000$ replicates (Supporting Information, Section~S3): its expected
count is $0.0003$ under $\HG$, $0.0004$ under $\HI$ and $0.028$ under $\HW$, with at
least one event in $0.03\%$, $0.04\%$ and $2.8\%$ of replicates respectively, so the
observed zero is the modal null outcome (two-sided $p = 1$ under each null). Adding the
rule as a sixth component of the joint statistic changes little, for two reasons. Because
the observed count lies within $0.03$ of its null mean, the sixth component contributes
essentially nothing to the observed distance, and $\DM$ is unchanged ($5.07$, $4.87$ and
$2.98$ under $\HG$, $\HI$ and $\HW$). At the same time, every null replicate that
contains an event lies beyond the observed vector in that component, so the few such
replicates can only raise the joint $p$-value: on the same paired replicates it moves
from $< .001$, $< .001$ and $.115$ to $< .001$, $< .001$ and $.139$. In the Holm family
the rule would contribute a $p$-value of 1. The conclusions are therefore unaffected by
its exclusion. The five-pattern family was nevertheless fixed after
inspection of the observed counts and should be regarded as data-informed rather than
pre-specified; the analyses reported below are accordingly described as primary rather
than confirmatory.

As a descriptive characterisation, each woman's within-woman SD of cycle length was
computed; among the 20 women with at least one Haflaga event the within-woman SD averaged 1.54 days
(SD across women 0.59), compared with 3.17 days (SD across women 1.75) among the 35 women
with no event of any type (Mann--Whitney $U = 133$,
...
longer than the minimum counts once (four equal intervals are one Haflaga event), and
a qualifying run that resumes after a discordant cycle counts again. To fix the
convention: $(29, 29)$ qualifies as Week ($29 = 4 \cdot 7 + 1$) but not as Haflaga;
...
\begin{description}
  \item[$\HG$ \textnormal{(global).}] All 1,554 intervals are
    exchangeable across women: the pooled values are permuted without replacement and
    re-assigned to women preserving each $n_i$. Tests whether the counts differ from what
    a homogeneous population would produce.
...
interval with probability $q \in \{0, .1, .2, .3\}$ and is otherwise drawn from the
...
To test whether the contrast between nulls is a property of the Marquette sample, the
complete analysis was repeated on an independent, openly deposited cohort: 3,324 cycles
from 581 women aged 18--40 with regular bleeding and no known subfertility, pooled from
three Creighton Model cohorts charted in 1990--2013 \citep{stanford2023data,najmabadi2020};
licence CC BY-NC. Preprocessing rules were fixed before any pattern was counted: the
recorded cycle length, exclusion of the 180 terminal conception cycles, splitting of a
woman's record at every date gap, and each woman's longest run of at least five
consecutive cycles, with no cycle-length criterion, giving 270 women and 2,139 cycles
(5--15 per woman). The counting rules, null generators and test statistics are the same
functions as in the main analysis, run with the same seed, $B$ and simulation sizes
through a dataset-agnostic driver that was first verified to reproduce every Marquette
result exactly; full results are in the Supporting Information (Section~S4) and the
\texttt{external\_replication} folder of the repository.
...
closely (Supporting Information, Figure~S4; largest difference in two-sided $p$-values 0.11, for
...
Week-Dilug     & 18 & 16.61 & 3.30 & $+0.42$ & $.774$ & $1.00$ & $.817$ \\
...
counts differ from what a homogeneous population would produce, not whether each woman's
\emph{ordering} produces a frequency of events different from what her own values
predict. $\HW$ asks the latter. Table~\ref{tab:stratified} and Figure~\ref{fig:stratified} give the results.
...
Week-Dilug     & 18 & 21.46 & 3.30 & $-1.05$ & $.370$ & $1.00$ & $.24$--$.58$ \\
Dilug-in-Dilug & 27 & 39.12 & 5.84 & $-2.07$ & $.038$ & $.192$ & $.014$--$.073$ \\
\midrule
\multicolumn{8}{@{}p{14.6cm}@{}}{Joint test (five patterns): $\DM = 2.97$, $p = .115$ (5,728 of 50,000 replicates at or beyond the observed value); split-batch $\DM = 2.99$, $p = .107$; leave-one-woman-out range of $p$: $.051$--$.207$.} \\
\bottomrule
\end{tabular}
\end{table}
...
result is exploratory. The joint test is non-significant ($\DM = 2.97$, $p = .11$) and
essentially unchanged in the split-batch form ($\DM = 2.99$, $p = .11$).
...
feature, whereas the global nulls erase it. Two further checks, computed from the
per-woman replicate counts, indicate that the concentration of events in a minority of
women is a property of their multisets rather than of their orderings (Supporting
Information, Section~S6). Within terciles of within-woman SD the joint $\HW$ test is
non-significant in every stratum ($p = .37$, $.79$ and $.25$), and the women who produce
the most events are those the null expects to: the rank correlation between a woman's
observed and expected event totals is $0.61$, no woman departs from her own null after
correction (1 of 118 with two-sided $p < .05$, against about 6 expected), and the share of
all events held by the six most productive women ($.24$) is at its null level ($.21$,
$p = .11$).
...
Under $\HG$ the five-pattern Mahalanobis distance is $\DM = 5.07$ ($p < .001$; 7 of
50,000 replicates at or beyond) and under $\HI$ it is $\DM = 4.87$ ($p < .001$; 16 of
50,000); the split-batch values are $5.07$ and $4.87$ with the same $p$-values. Under
$\HW$, $\DM = 2.97$, $p = .115$ (Table~\ref{tab:stratified};
...
are well conditioned (condition numbers 4.6, 4.4 and 5.2 for $\HG$, $\HI$, $\HW$) with
...
$\DM = 4.50$ ($p < .001$), $4.39$ ($p < .001$) and $2.72$ ($p = .059$) under $\HG$,
...
reject between 2.8\% and 4.8\% of the time (conservative because the counts are
discrete), the Holm family between 3.0\% and 3.6\%, and the joint test between 4.6\% and
5.3\% at $\alpha = .05$ and between 0.7\% and 1.0\% at $\alpha = .01$. The plug-in and
...
\midrule
$\HG$  & .029 & .047 & .034 & .034 & .046 & .034 & .050 (.010) & .053 (.009) \\
$\HI$  & .048 & .038 & .040 & .028 & .046 & .030 & .048 (.007) & .046 (.008) \\
$\HW$  & .043 & .041 & .043 & .032 & .040 & .036 & .052 (.009) & .051 (.010) \\
\bottomrule
\end{tabular}
\end{table}
...
tests reject at nominal rates under these heterogeneous marginals (joint 5.0\% and 6.0\%, SE 1.2\%).
...
53\% of datasets and by the Holm family in 27\%; at $\rho = 0.5$ the joint test rejects
in 99.8\% and at $\rho = 0.75$ always. Exact-repetition persistence is detected far more
...
suppresses second-order progressions), and Week and Week-Dilug have the least power
against autoregression, as expected from their definitions.
...
\midrule
AR(1) $\rho$       & 0    & $-0.1$ & .04 & .05 & .04 & .04 & .04 & .03 & .05 \\
                   & 0.25 & $+5.1$ & .18 & .24 & .10 & .09 & .18 & .27 & .53 \\
                   & 0.50 & $+13.8$ & .76 & .74 & .35 & .39 & .44 & .89 & .998 \\
                   & 0.75 & $+33.3$ & 1.00 & .97 & .78 & .77 & .73 & 1.00 & 1.00 \\
\midrule
Persistence $q$    & 0    & $+0.2$ & .04 & .04 & .04 & .03 & .06 & .05 & .06 \\
                   & 0.1  & $+24.1$ & .96 & .19 & .61 & .54 & .04 & .94 & .98 \\
                   & 0.2  & $+52.7$ & 1.00 & .60 & .96 & .95 & .04 & 1.00 & 1.00 \\
                   & 0.3  & $+80.4$ & 1.00 & .93 & .99 & .99 & .16 & 1.00 & 1.00 \\
\bottomrule
\end{tabular}
\end{table}
...
the joint test rejected in 53\% of datasets; at $\rho = 0.5$ (about 14 additional
events) it rejected in 99.8\%. Under the persistence alternative, one exact repetition
...
$(19, 51, 21, 16, 21)$, all $z$ remain negative ($-1.18$, $-0.32$, $-0.89$, $-0.83$,
$-1.78$), no marginal test approaches significance (smallest two-sided $p = .080$, for
Dilug-in-Dilug), and the joint test gives $\DM = 2.52$, $p = .27$. Leaving out any single
woman moves the joint $\HW$ $p$-value within $.051$--$.207$ and never below $.05$
...
In the Utah cohort the observed counts are $(41, 76, 35, 42, 41)$. Under $\HG$ all five
exceed their null means (Haflaga $z = 8.33$, no replicate reached 41; Dilug, Week-Dilug
and Dilug-in-Dilug Holm-adjusted $p$ $.010$--$.024$) and $\DM = 10.04$
(Table~\ref{tab:ext}). Under $\HW$ every $|z| \leq 1.13$, all two-sided $p \geq .31$, and
$\DM = 1.22$ ($p = .92$; split-batch $1.21$, $.92$); the Dilug-in-Dilug deficit seen in
the Marquette data is absent ($z = -0.08$). The conclusion is unchanged by leaving any
woman out (joint $p$ $.86$--$.97$), truncation to twelve cycles ($\DM = 1.33$, $p = .88$)
or restriction of the support to 18--54 days ($\DM = 1.20$, $p = .92$); empirical size is
at or below nominal and the power of the $\HW$ tests is comparable to that in the
Marquette data (Supporting Information, Section~S4). The per-woman checks of
Section~\ref{sec:within} also hold: every regularity stratum is null (joint $p \geq .55$),
the rank correlation between observed and expected per-woman totals is $0.70$, and the
concentration of events among women is at its null level (Section~S6).

\begin{table}[!htbp]
\centering
\caption{Joint five-pattern test in the Marquette sample (118 women, 1,554 cycles) and
  the Utah replication cohort (270 women, 2,139 cycles); $B = 50{,}000$ per null.
  $p$-values are Monte Carlo, $(b+1)/(B+1)$.}
\label{tab:ext}
\smallskip
\begin{tabular}{L{2.2cm} C{3.4cm} C{3.4cm} C{3.0cm}}
\toprule
\textbf{Null} & \textbf{Marquette} $\DM$ ($p$) & \textbf{Utah} $\DM$ ($p$) & \textbf{Utah Haflaga} $z$ \\
\midrule
$\HG$  & 5.07 ($<.001$) & 10.04 ($<.001$) & $+8.33$ \\
$\HI$  & 4.87 ($<.001$) & 9.56 ($<.001$) & $+8.20$ \\
$\HW$  & 2.97 ($.115$)  & 1.22 ($.916$)  & $+1.13$ \\
\bottomrule
\end{tabular}
\end{table}
...
($\DM = 5.07$, $p < .001$) and unremarkable under within-woman exchangeability
($\DM = 2.97$, $p = .11$). The global result is real and is not an artefact of the
...
across women, treat this as excess. This account is supported by the data, most directly
by the per-woman checks of Section~\ref{sec:within}, in which the women who produce the
most events are those whose multisets predict the most under $\HW$; it is not uniquely
identified by them, and other mechanisms that leave the multiset-conditional
distribution of these counts unchanged cannot be excluded.

The replication sharpens this account. Writing $z_G = (\mu_W - \mu_G)/\sigma_G +
(O - \mu_W)/\sigma_G$, the heterogeneity gap for Haflaga is nearly identical in the two
cohorts (6.28 and 6.76) while the within-woman residual is $-1.79$ and $+1.57$: the larger
global displacement in the Utah cohort arises because its counts sit at, rather than
below, their within-woman expectations, and from its larger sample; the within-woman null
gives the same answer in both (Supporting Information, Table~S10).
...
the 28-day cycle ($H = 29$) repeated twice, because longer weekly anchors (35, 42 days)
are rare (9.6\% of cycles are $\geq 35$ days). Every Week-Dilug event is at the anchor
$H = 30$, and the events include the 3 Haflaga-30 sequences by containment.
...
Counts are invariant to the $H_k = L_k + 1$ relabelling provided the Week and Week-Dilug
anchor sets are shifted accordingly (verified: 26 Haflaga, 18 Week-Dilug under both
conventions; computing on $L_k$ with the unshifted anchors $\{23, 30, 37, \ldots\}$ gives
13 Week-Dilug events). The
distribution of intervals declines smoothly through the anchor ($H = 29$: 225 cycles;
...
\paragraph{Scope.}
Calendar dates are absent, precluding the 11 date-dependent rule types; body-sign
patterns (\textit{vesset haguf}) are invisible to cycle-length data. The sixth
interval-computable rule, Haflaga Chozer Chalila, had no observed occurrence and a
negligible null expectation (Section~\ref{sec:data}). The sample consists of non-Jewish, regularly cycling, married
...
population need not transfer to observant Jewish women \citep{ross2022phd}. Both
cohorts were selected for regular cycles at entry, and follow-up in the Utah cohort is
short (5--15 cycles per woman). Results should not be generalised without replication in
...
\begin{figure}[!htbp]
  ...
  \caption{Frequency of each interval value $H_k = L_k + 1$ in the analysed sample.
    ...
    the Week-Dilug anchor at which all 18 events occur. Counts decline smoothly through these values
...
from the released dataset. The second dataset, used for the external replication, is a
de-identified public deposit \citep{stanford2023data} analysed as secondary data under
its licence. Separate ethics approval for secondary analysis was not required under the
...
The external replication uses the openly deposited dataset of \citet{stanford2023data},
held at The Hive, University of Utah, under a CC BY-NC licence
(\doi{10.7278/S50d-4gxs-s4hj}). The raw
deposit, the pre-specified preprocessing script and its output with SHA-256 checksums,
the analysis code, the cached replicates, the simulation output, the figures and the
reports are in the \texttt{external\_replication} folder of the repository.
