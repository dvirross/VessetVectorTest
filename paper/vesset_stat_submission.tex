...
explored through exploratory pairwise Spearman rate-based correlation tests
(Haflaga $\times$ Dilug-in-Dilug: $r_s = 0.27$, $p_{\text{perm}} = .003$, Holm-adj $p = .030$).
...
rather than binary indicators addresses the cycle-count exposure confound: women
with more cycles have more opportunities to satisfy any pattern, and normalising
by cycle count places all women on equal footing. Two-sided permutation $p$-values
...
The data are consistent with the analytical predictions: both structurally predicted
pairs show positive correlation. After Holm correction, Haflaga $\times$
Dilug-in-Dilug survives ($r_s = 0.27$, $p_{\text{perm}} = .003$, adj $p = .030$),
confirming the polynomial-hierarchy proximity relationship. Haflaga $\times$
Week-Dilug shows a positive association ($r_s = 0.23$, $p_{\text{perm}} = .010$)
consistent with the structural prediction, but does not survive Holm correction
(adj $p = .090$). All eight remaining pairs are non-significant
(all adj $p \geq .206$), consistent with the absence of analytical predictions
for those pairs. Figure~\ref{fig:chisq} visualises the full correlation structure.\footnote{A
...
\begin{figure}[H]
  ...
  \caption{Pairwise Spearman correlation structure between vesset pattern types.
    ...
    Holm correction ($r_s = 0.27$, adj $p = .030$),
    consistent with the polynomial-hierarchy structure in Section~\ref{sec:mathdep}.}
  \label{fig:chisq}
\end{figure}
