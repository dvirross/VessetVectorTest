...
\For{$m \in \{G, M, W\}$}
  ...
  \State $p_j^{(m)} \leftarrow 2\min\!\bigl((|\{r : S_j^{(m,r)} \geq O_j\}|+1)/(B+1),\;(|\{r : S_j^{(m,r)} \leq O_j\}|+1)/(B+1)\bigr)$
...
\begin{figure}[H]
  ...
  \caption{Empirical null distributions (shown as smoothed density estimates) vs.\ observed counts for each vesset pattern
    ...
    The vertical dotted line marks the observed count; the shaded regions indicate
    departure from the null in either direction. Only Haflaga shows clearly displaced
    observed value; Dilug-in-Dilug falls below its null mean.}
  \label{fig:nulldists}
\end{figure}
...
(Holm-adjusted $p = .0003$). Dilug is not significant ($p = .087$, Holm adj $p = .346$) and should be regarded as an exploratory
...
(all Holm-corrected $p \geq .198$; joint $D_M = 2.95$, $p = .121$), and all observed counts fall
