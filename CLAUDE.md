...
`notebooks/analysis.ipynb` is generated by `scripts/build_notebook.py` — **edit the builder, not the notebook**, then:
```bash
...
- [x] Patient Consent wording now cites the Marquette archive record: "anonymized for dissemination", "data reuse was agreed to by subjects in the consent form" (data_nfp/7 "Rights, Permissions, & Privacy" field, confirmed verbatim by the author on 2026-09-16; exact text also recorded in the `fehring2012data` bib note)
